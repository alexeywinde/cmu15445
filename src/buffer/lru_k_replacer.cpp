...
LRUKReplacer::LRUKReplacer(size_t num_frames, size_t k) : replacer_size_(num_frames), k_(k) { max_size = num_frames; }
...
void LRUKReplacer::SetEvictable(frame_id_t frame_id, bool set_evictable) {
  ...
  if (frame_evict_.at(frame_id) == true && set_evictable == false) replacer_size_--;
  if (frame_evict_.at(frame_id) == false && set_evictable == true) replacer_size_++;
}
...
void LRUKReplacer::Remove(frame_id_t frame_id) {
  ...
  if (iter == frame_evict_.end()) return;
